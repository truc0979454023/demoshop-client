  ...
  useEffect(() => {
    ...
    const getProductsNew = async () => {
      const res = await axios.get(
        `/api/products?limit=${page * 6}&sort=-createdAt`
      );
    ...
    const getProductsSaleOff = async () => {
      const res = await axios.get(
        `/api/products?limit=${page * 6}&sort=-saleOff`
      );
    ...
    <>
      ...
      {productsSaleOff?.length !== 0 ? (
        <div className="show-products">
              ...
              return (
                <div key={product._id} className="show-products-item">
                  <ProductItem
                    key={product._id}
                    product={product}
                    setProducts={setProductsSaleOff}
          ...
          <Link
            to="/products"
            onClick={() => setSort("sort=-saleOff") && setCategory("")}
          >
      ...
      {productsSale.length !== 0 ? (
        <div className="show-products ">
          ...
          <Link
            to="/products"
            onClick={() => setSort("sort=-sold") && setCategory("")}
          >
            <p className="link show-products-title">Load more</p>
          </Link>
        </div>
      ...
      {productsNew.length !== 0 ? (
        <div className="show-products ">
          ...
          <Link
            to="/products"
            onClick={() => setSort("sort=-createdAt") && setCategory("")}
          >
            <p className="link show-products-title">Load more</p>
          </Link>
        </div>
