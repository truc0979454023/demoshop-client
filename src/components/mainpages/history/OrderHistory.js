...
function OrderHistory() {
  ...
  useEffect(() => {
    if (userProfile) setData({ ...data, name: userProfile.name });
  }, [userProfile, data]);

  useEffect(() => {
    setTemp(history);
    if (filter === "true") {
      setTemp((history) => history.filter((item) => item.status === true));
    }
    if (filter === "false") {
      setTemp((history) => history.filter((item) => item.status === false));
    }
  }, [filter, history]);
    ...
    if (name !== userProfile.name && name) {
      try {
        const res = await axios.patch(
          "/user/update_info",
          { data: { ...data, email: userProfile.email } },
          {
  ...
  return (
    <div className="profile">
      <div className="user-profile">
        ...
        <button className="user-profile-btn" onClick={handleSubmit}>
          {" "}
          Update
        </button>
      </div>
            ...
            <thead>
              <tr>
                ...
                <th>
                  ...
                  <select
                    value={filter}
                    style={{ marginLeft: "4px", border: "none", width: "20px" }}
                    className={
                      filter === "true"
                        ? "check-user"
                        : filter === "false"
                  ...
                  >
                    <option value=""></option>
                    <option value="true" className="check-user">
                      V
                    </option>
                    <option value="false" className="times-user">
                      X
                    </option>
                  </select>
                </th>
            ...
            <tbody>
              {temp.map((items) => (
                <tr key={items._id}>
                  ...
                  <td
                    onClick={() =>
                      handleChangeDelivery(items._id, items.status)
                    }
