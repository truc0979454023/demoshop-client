    ...
    const temp01 = months.map((month, index) => {
      return history.reduce((array, item) => {
        if (new Date(item.createdAt).getMonth() === index) {
          const totalSale = item.cart.reduce(
            (sum, item) => (sum += item.product.price * item.product.quantity),
            0
          );

          const totalSold = item.cart.reduce(
    ...
    const temp01 = history.reduce((array, hs) => {
      if (new Date(hs.createdAt).getFullYear() === year) {
        const temp02 = hs.cart.reduce((array, item) => {
          array.push({
            ...item.product,
            monthCreated: new Date(hs.createdAt).getMonth() + 1,
          });
          return array;
        }, []);
        array.push(temp02);
      ...
      <div>
        ...
        <div style={{ padding: "20px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end" }}>
            ...
            <button
              onClick={() => exportTableToExcel("tblData", "data")}
              style={{
                background: "gray",
                padding: "4px 24px",
                borderRadius: "4px",
