...
function Cart() {
  ...
  const handlePayment = () => {
    const number = cart.reduce((count, c) => {
      if (
        (c.product.size.length !== 0 && c.size === "") ||
        (c.product.color.length !== 0 && c.color === "")
      )
        return (count += 1);
      return count;
    }, 0);
    ...
    cart.forEach((cartItem) => {
      array.push(
        ...productAll.filter((productItem) => {
          return (
            productItem.active === false &&
            productItem._id === cartItem.product._id
          );
        })
      );
    });
          ...
          <div className="box-detail-cart">
            ...
            <div className="content">
              <div className="content-price-sold">
                <h3 style={{ fontSize: "24px" }} className="price">
                  $ {cItem.product.price}{" "}
                </h3>
                <div className="size-color">
                  <p style={{ fontSize: "20px" }}>Size:</p>
                  {cItem.product.size
                    ? cItem.product.size.map((s) => (
                        <p
                          key={s}
                          className={`cart-size-item ${isActiveSize(
                            cItem.product._id,
                            s
                          )}`}
                          onClick={() => onSize(cItem.product._id, s)}
                        >
                          {s}
                        </p>
                      ))
                ...
                <div className="size-color">
                  {cItem.product.color.length !== 0 ? (
                    <>
                      ...
                      {cItem.product.color
                        ? cItem.product.color.map((c) => (
                            <p
                              key={c}
                              className={`cart-color-item ${isActiveColor(
                                cItem.product._id,
                                c
                              )}`}
                              onClick={() => onColor(cItem.product._id, c)}
                            >
                              {c}
                            </p>
                          ))
